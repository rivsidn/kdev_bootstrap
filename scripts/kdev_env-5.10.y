...
export KDEV_DOCKER_IMAGE=ubuntu-22.04-amd64
export KDEV_QEMU_IMAGE=/opt/ubuntu-rootfs/ubuntu-22.04-amd64-rootfs.img
	...
	case "$mode" in
		...
		internet)
			qemu-system-x86_64 -kernel $KDEV_KERNEL -hda $KDEV_QEMU_IMAGE \
				...
				-append "root=/dev/sda rw init=/usr/sbin/init console=tty0" -m 256M
			;;
		bridge)
			# 桥接模式（需要 root 权限）
